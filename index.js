...
const authorize = (role) => {
    return async (req, res, next) => {
        try {
            ...
            if (role === 'user' && !email.endsWith('@kpriet.ac.in')) {
                return res.status(403).json({ error: 'User must belong to KPRIET domain' });
            }

            if (isAdmin && admin.department) {
                req.user.department = admin.department;
            }
...
const validatePeriods = async (year, periods, dateStr) => {
    if (!periods || periods.length === 0) {
        return { valid: false, errorMsg: 'No periods selected' };
    }
    ...
    if (timings.length !== periods.length) {
        return { valid: false, errorMsg: 'Invalid period selection' };
    }

    const requestDate = new Date(dateStr);
    const today = new Date();

    const reqDate = new Date(requestDate.getFullYear(), requestDate.getMonth(), requestDate.getDate());
...
app.post('/api/od/request', authenticate, authorize('user'), async (req, res) => {
    try {
        const { year, periods, date, user_id } = req.body;
        ...
        const validation = await validatePeriods(year, periods, date);
        if (!validation.valid) {
            return res.status(400).json({ error: validation.errorMsg });
        }
...
app.get('/api/reports/export', authenticate, authorize('admin'), async (req, res) => {
    try {
        ...
        const rows = data.map(r =>
            `"${r.reg_number}","${r.department}","${r.year}","${r.od_category}","${r.date}","${r.status}"`
        ).join('\n');
